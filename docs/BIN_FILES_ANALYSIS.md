...
- 将控制权直接移交给内核（通过加载 kernel.bin 到 0x80200000）
...
### 最终作用
- 直接作为内核二进制文件，由 QEMU 的 `-kernel` 参数加载
- 操作系统的主要逻辑
...
2. **加载内核镜像**
   - RustSBI 将 `kernel.bin` 加载到内存 `0x80200000`
...
- 内核二进制文件存放在 `build/` 目录（直接作为 QEMU 的 -kernel 参数）
- 已移除独立的 bootloader 代码和冗余的 os.img，内核直接从 0x80200000 开始
