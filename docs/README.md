# Chronos OS - 文档索引

## 🚀 快速开始

- [README.md](../README.md) - 项目主文档
- [QUICKREF.md](../QUICKREF.md) - 快速参考手册
- [docs/QUICKSTART.md](QUICKSTART.md) - 快速开始指南

## 📚 核心文档

### 项目概览
- [PROJECT_STATUS_REPORT.md](PROJECT_STATUS_REPORT.md) - 项目状态报告
- [CHANGELOG.md](CHANGELOG.md) - 更新历史

### 架构设计
- [ARCHITECTURE_DESIGN.md](ARCHITECTURE_DESIGN.md) - 系统架构设计
- [KERNEL_STRUCTURE.md](KERNEL_STRUCTURE.md) - 内核模块结构

### 功能实现
- [MEMORY_MANAGEMENT.md](MEMORY_MANAGEMENT.md) - 内存管理系统
- [USER_MODE_IMPLEMENTATION.md](USER_MODE_IMPLEMENTATION.md) - 用户态实现

## 🔧 开发文档

### 构建与运行
- [RUSTSBI_BUILD_PROCESS.md](RUSTSBI_BUILD_PROCESS.md) - RustSBI 构建流程

### 技术细节
- [MEMORY_SIZE_EXPLANATION.md](MEMORY_SIZE_EXPLANATION.md) - 内存配置说明
- [IMPLEMENTATION_SUMMARY.md](IMPLEMENTATION_SUMMARY.md) - 实现总结

## 📁 文档结构

```
docs/
├── QUICKSTART.md                 # 快速开始
├── QUICKREF.md -> ../QUICKREF.md # 快速参考 (根目录)
├── README.md -> ../README.md     # 主文档 (根目录)
├── CHANGELOG.md                  # 变更日志
├── PROJECT_STATUS_REPORT.md      # 项目状态
├── ARCHITECTURE_DESIGN.md        # 架构设计
├── KERNEL_STRUCTURE.md           # 内核结构
├── MEMORY_MANAGEMENT.md          # 内存管理
├── USER_MODE_IMPLEMENTATION.md   # 用户态实现
├── RUSTSBI_BUILD_PROCESS.md      # RustSBI 构建
├── MEMORY_SIZE_EXPLANATION.md    # 内存配置
└── IMPLEMENTATION_SUMMARY.md     # 实现总结
```

## 🎯 阅读建议

### 新手入门
1. [README.md](../README.md) - 了解项目
2. [QUICKSTART.md](QUICKSTART.md) - 快速开始
3. [QUICKREF.md](../QUICKREF.md) - 常用命令

### 深入学习
1. [ARCHITECTURE_DESIGN.md](ARCHITECTURE_DESIGN.md) - 系统架构
2. [KERNEL_STRUCTURE.md](KERNEL_STRUCTURE.md) - 内核结构
3. [MEMORY_MANAGEMENT.md](MEMORY_MANAGEMENT.md) - 内存管理

### 项目状态
1. [PROJECT_STATUS_REPORT.md](PROJECT_STATUS_REPORT.md) - 当前状态
2. [CHANGELOG.md](CHANGELOG.md) - 开发历史
3. [IMPLEMENTATION_SUMMARY.md](IMPLEMENTATION_SUMMARY.md) - 实现总结

---

**文档版本**: v0.2.0
**最后更新**: 2026-01-15